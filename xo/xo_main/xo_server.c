...
int create_xo_player_table_entry(int num_player)
{
  ...
  xo_player = realloc(xo_player, (num_player + 2) * sizeof(XO_Player) );
...
-------------------------------------------------------*/
int close_connection(connection *cptr)
{
  XODL_SER("Method called.\n"); 
  close(cptr->conn_fd);
...
/*-----------------------------------------------------
Function to delete Player info from Table

-----------------------------------------------------*/
int delete_xo_player_data(connection *cptr)
{
  int row_num;
  XODL_SER("Method called.\n");
  XODL_SER("Number of Player = [%d].\n", num_player);
  
  //if (cptr->idx == (num_player-1))
  //  num_player--;
  //else {
    for (row_num = cptr->idx; row_num < (num_player -1); row_num++)
    {
      XODL_SER("row_num = [%d]\n", row_num);
      xo_player[row_num] = xo_player[row_num + 1];
      xo_player[row_num].cptr->idx--;
    }  
  num_player--;
  //}
...
/*------------------------------------------------
Function to check status of remote user

------------------------------------------------*/
int check_remote_user_status(connection *cptr)
{
  int remote_player;
  char buff[500];
  XODL_SER("Method called.\n");
  remote_player = atoi(cptr->read_buff + 13);
  XODL_SER("remote player = [%d]\n", remote_player);
  if (xo_player[remote_player-1].status == ONLINE) {
    sprintf(buff, "CHAT_REQUEST:%s", xo_player[cptr->idx].name);
    ...
    XODL_SER("conn_fd = [%d]\n", xo_player[remote_player-1].cptr->conn_fd);
    write_to_client(xo_player[remote_player-1].cptr, buff, strlen(buff));
  }
  else {
    sprintf(buff, "REQUEST_REJECTED");
    write_to_client(cptr, buff, strlen(buff));
...
int check_remote_player_status(connection *cptr)
{
  ...
  remote_player = atoi(cptr->read_buff + 13);
  if (xo_player[remote_player-1].status == ONLINE){
    sprintf(buff, "GAME_REQUEST:%s|GAME_TYPE:%s", xo_player[cptr->idx].name, (xo_player[cptr->idx].game_type == 1) ? "3X3" : "4X4");
    xo_player[cptr->idx].status = BUSY;
    xo_player[remote_player-1].status = BUSY;
    xo_player[remote_player-1].peer = &xo_player[cptr->idx];
    xo_player[cptr->idx].peer = &xo_player[remote_player-1];
...
----------------------------------------------------*/
int proc_user_chat(connection *cptr)
{
  XODL_SER("Method called.\n");
  write_to_client(xo_player[cptr->idx].peer->cptr, cptr->read_buff);
...
int set_game_type(connection *cptr)
{
  ...
  XODL_SER("Game_Type = [%d]\n", xo_player[cptr->idx].game_type);
...
int proc_client_req(connection *cptr)
{
  ...
  if ( strncmp(cptr->read_buff, "GAME_TYPE:", 10) == 0)
    set_game_type(cptr);
  else if ( strncmp(cptr->read_buff, "ONLINE_PLAYERS", 14) == 0 ||
            strncmp(cptr->read_buff, "ONLINE_USERS", 12) == 0 )                                                    
    get_players_list(cptr);
  else if ( strncmp(cptr->read_buff, "STATUS:ONLINE", 13) == 0)
    xo_player[cptr->idx].status = ONLINE;
  else if ( strncmp(cptr->read_buff, "GAME_REQUEST", 12) == 0)
    check_remote_player_status(cptr);
  else if ( strncmp(cptr->read_buff, "CHAT_REQUEST", 12) == 0)
    check_remote_user_status(cptr);
  else if (strncmp(cptr->read_buff, "REQUEST_ACCEPTED", 16) == 0)
    start_game(cptr);    
  else if (strncmp(cptr->read_buff, "MOVE:", 5) == 0)
    proc_players_move(cptr);
  else if (strncmp(cptr->read_buff, "CHAT:", 5) == 0)
    proc_user_chat(cptr);
  else if (strncmp(cptr->read_buff, "CLOSE", 5) == 0)
    delete_xo_player_data(cptr);
...
int main(int argc, char *argv[])
{
  ...
  int i = 0, j = 0, sock_fd, efd, num_events, con_fd, data_read;
  char buff[1024] = "aaaa";
  connection *cptr;
  XODL_SER("Method called.\n");
