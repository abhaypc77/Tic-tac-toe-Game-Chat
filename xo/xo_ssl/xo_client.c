...
int create_tcp_connection()
{
  ...
  xo_init_ssl();
  xo_client.bio = BIO_new_socket(sock_fd, BIO_NOCLOSE);
  SSL_set_bio(xo_client.ssl, xo_client.bio, xo_client.bio);
  SSL_accept(xo_client.ssl);
...
int write_to_server(int conn_fd, char *buff, int size)
{
  ...
  while(1)
  {
    bytes_written = SSL_write(xo_client.ssl, buff, strlen(buff));
...
int read_from_server(int conn_fd)
{
  ...
  while(1)
  {
    bytes_read = SSL_read(xo_client.ssl, xo_client.read_buff + total_bytes_read, 1024);
...
int create_player_connection(int player)
{
  char req[100];
  XODL_CLI("Method called.\n");
  sprintf(req, "START_GAME:%d", player);
  write_to_server(xo_client.conn_fd, req, strlen(req));
  //read_from_server(xo_client.conn_fd);
...
int main(int argc, char *argv[])
{
  ...
  xo_client_init();
  printf("\n");
  printf("1. PLay Online\n");
  printf("2. Play Offline\n\n");
  printf("Enter your choice...");
  scanf("%d",&choice);
